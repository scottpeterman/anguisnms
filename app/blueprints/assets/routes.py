...
def api_device_capture_content(device_id, capture_type):
    """API endpoint to get capture file content"""
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            ...
            if not capture:
                return jsonify({'error': 'Capture not found', 'status': 'error'}), 404
            ...
            if not os.path.exists(file_path):
                return jsonify({'error': 'Capture file not found on disk', 'status': 'error'}), 404
